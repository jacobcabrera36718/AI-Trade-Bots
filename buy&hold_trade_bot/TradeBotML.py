...
def simulate_news_sentiment(symbol: str, current_price: float, recent_performance: float) -> Tuple[float, str]:
    """
    EXTREME BUYING focused sentiment - BUY EVERYTHING!
    """
    try:
        # EXTREME BUYING bias - almost always generate buy signals
        if recent_performance < 0:  # ANY decline = STRONG buy
            confidence = np.random.uniform(0.8, 0.95)  # Very high confidence on dips
            sentiment_label = "positive"  # All declines = buy opportunities
            logger.info(f"MEGA DIP BUY! {symbol}: STRONG BUY ({confidence:.3f}) on {recent_performance:.1f}% decline")
       
        elif recent_performance > 0:  # Even gains = buy (momentum)
            confidence = np.random.uniform(0.6, 0.8)  # Still high confidence
            sentiment_label = "positive"  # Gains = momentum buying
           
        else:  # Neutral = BUY (default bullish)
            confidence = np.random.uniform(0.7, 0.9)  # High confidence always
            sentiment_label = "positive"  # Always bullish
       
        # BOOST ALL SIGNALS - Make them even stronger
        confidence = min(confidence + 0.1, 0.95)
       
        logger.info(f"EXTREME BUY sentiment for {symbol}: {sentiment_label} ({confidence:.3f}) "
                   f"based on {recent_performance:.1f}% performance")
       
        return float(confidence), sentiment_label
       
    except Exception as e:
        logger.error(f"Error simulating extreme buy sentiment: {e}")
        return 0.9, "positive"  # Default to maximum buy signal
