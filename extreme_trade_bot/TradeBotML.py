...
def simulate_news_sentiment(symbol: str, current_price: float, recent_performance: float) -> Tuple[float, str]:
    """
    ULTRA-AGGRESSIVE sentiment simulation - BUY EVERY DIP!
    """
    try:
        # EXTREME dip-buying sentiment
        if recent_performance < -0.5:  # Even small dips trigger strong buy signals
            confidence = np.random.uniform(0.7, 0.95)  # Very high confidence
            sentiment_label = "positive"  # Contrarian - bad performance = buy signal
            logger.info(f"DIP DETECTED! {symbol}: STRONG BUY ({confidence:.3f}) on {recent_performance:.1f}% decline")
       
        elif recent_performance < 0:  # Any decline at all
            confidence = np.random.uniform(0.6, 0.9)
            sentiment_label = "positive"  # Buy the dip
           
        elif recent_performance > 2:  # Strong gains - maybe take profits
            confidence = np.random.uniform(0.4, 0.7)
            sentiment_label = np.random.choice(["positive", "negative"], p=[0.3, 0.7])
           
        else:  # Neutral/small gains - still biased to buy
            confidence = np.random.uniform(0.5, 0.85)
            sentiment_label = "positive"  # Default bullish
       
        # Extra boost for ultra-aggressive mode
        confidence = min(confidence + 0.1, 0.95)
       
        logger.info(f"ULTRA-AGGRESSIVE sentiment for {symbol}: {sentiment_label} ({confidence:.3f}) "
                   f"based on {recent_performance:.1f}% performance")
       
        return float(confidence), sentiment_label
       
    except Exception as e:
        logger.error(f"Error simulating ultra-aggressive sentiment: {e}")
        return 0.9, "positive"  # Default to very strong buy signal
